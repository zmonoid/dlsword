...
from utils.utils import get_loader, finetune, play_data
...
if args.test is None:
    ...
    plist = [Process(target=train_session, args=(idx, train_index, val_index)) for idx, (train_index, val_index) in enumerate(kf)]
    for p in plist:
        p.start()
        p.join()
    ...
    if args.vis:
        test_loader = get_loader(imgs, config, 'vis')
        play_data(test_loader, model, config, "non-inv:%.2f, inv:%.2f")
    else:
        test_loader = get_loader(val_list, config, 'val')
        softmax = nn.Softmax()
        probs_list = []
        names_list = []
